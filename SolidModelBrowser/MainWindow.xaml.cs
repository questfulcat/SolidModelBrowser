    ...
    public partial class MainWindow : Window
    {
        ...
        private void LoadingTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    clearView();
                    textBlockInfo.Text = Import.ExceptionMessage;
                    return;
                }
                ...
                camera.DefaultPosition(modelSize.Length);

                if (currentImport.InitialXRotationNeeded) addRotation(Axis.X, 90.0);

                if (ButtonShowInfo.IsChecked.Value) showModelInfo();
                else textBlockInfo.Text = null;
            }
            catch (Exception exc)
            {
                loadingTimer.Stop();
                clearView();
                textBlockInfo.Text = exc.Message;
            }
        }
        ...
        void loadFile(string filename)
        {
            ...
            try
            {
                ...
                loadingTimer.Start();
                textBlockInfo.Text = "loading...";

                currentTask.Start();
                
                //textBlockInfo.Text = $"{lookCenter.X}, {lookCenter.Y}, {lookCenter.Z}";
            }
            catch (Exception exc)
            {
                textBlockInfo.Text = exc.Message;
        ...
        {
            if (e.Key == Key.A) addRotation(Axis.Z, 90.0);
            if (e.Key == Key.D) addRotation(Axis.Z, -90.0);
            if (e.Key == Key.S) addRotation(Axis.X, 90.0);
            if (e.Key == Key.W) addRotation(Axis.X, -90.0);
            if (e.Key == Key.E) addRotation(Axis.Y, 90.0);
            if (e.Key == Key.Q) addRotation(Axis.Y, -90.0);
